...
public class Shard {
    ...
    public void writeObject(Object e, Object key, int parentRecordIndex) {
        ...
        EncodeDataContainer ba = EncodeDataContainerFactory.newContainer(null,key,db.getEncoderType(),db.getTypeDescriptorsContainer().getTypeDescriptorByClass(clazz));
    ...
    public Object deleteObject(Class<?> clazz,Object key) {
        ...
        EncodeDataContainer ba = EncodeDataContainerFactory.newContainer(data,key,db.getEncoderType(),db.getTypeDescriptorsContainer().getTypeDescriptorByClass(clazz));
    ...
    public Object readObject(Class<?> clazz, int recordIndex) {
        ...
        EncodeDataContainer ba = EncodeDataContainerFactory.newContainer(data,null,db.getEncoderType(),db.getTypeDescriptorsContainer().getTypeDescriptorByClass(clazz));
        if(data==null || dataPersister.isDeleted(data)){
            ba.getEncoder().encodeNULL(ba);
            ba.resetLocation();
        }
    ...
    public void readSubObjects(EncodeDataContainer ba, TypeDescriptor parentTable,int parentRecordIndex) {
        for (AttributeDescriptor col : parentTable.getChildren().keySet()) {
            ...
            if(dataPersister==null){
                EncodeDataContainer subBA = EncodeDataContainerFactory.newContainer(null, null, db.getEncoderType(),subTable);
            ...
            if (data != null) {
                for (int i = 0; i < data.length; i++) {
                    if(!dataPersister.isDeleted(data[i])){
                        EncodeDataContainer subBA = EncodeDataContainerFactory.newContainer(data[i],null,db.getEncoderType(),subTable);
                        ba.addSubElementData(subTable.getClassCode(), subBA,null);
